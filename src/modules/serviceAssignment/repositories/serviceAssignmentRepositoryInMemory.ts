...
export class ServiceAssignmentRepositoryInMemory implements ServiceAssignmentRepository {
  ...
  async save(serviceAssignment: ServiceAssignment): Promise<void> {
    const serviceAssignmentIndex = this.serviceAssignments.findIndex(
      currentServiceAssignment => currentServiceAssignment.id === serviceAssignment.id
    );
