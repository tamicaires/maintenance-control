...
describe('Get Many Service Assignments' ,() => {
  beforeEach(() => {
    ...
    getManyServiceAssignments = new GetManyServiceAssignments(serviceAssignmentRepositoryInMemory); 
  });

  it('Should be able to get many service assignments', async () => {
    const serviceAssignments = [...new Array(10)].map(() => makeServiceAssignment({}));
  ...
  it('Should be able to control page service assignments', async () => {
    const serviceAssignments = [... new Array(10)].map((_, index) => makeServiceAssignment({
