...
describe('Edit Carrier', () => {
  ...
  it('Should be able to edit carrier', async () => {
    ...
    expect(carrierRepositoryInMemory.carriers[0].carrierName).toEqual(
      carrierNameChanged,
    );

    expect(carrierRepositoryInMemory.carriers[0].managerName).toEqual(
      managerNameChanged,
    );

    expect(carrierRepositoryInMemory.carriers[0].managerPhone).toEqual(
      managerPhoneChanged,
    );
  });
