...
export const defineTrailerAbilities = (
  builder: AbilityBuilder,
  companyId: string,
  userRoles: TRole[]
) => {
  builder.can(Action.Read, 'Trailer', { companyId });

  if (userRoles.includes(RoleEnum.SUPER_ADMIN)) {
    builder.can(Action.Manage, 'Trailer', { companyId });
    builder.can(Action.Read, 'Trailer', { companyId });
    builder.can(Action.Create, 'Trailer', { companyId });
    builder.can(Action.Update, 'Trailer', { companyId });
    builder.cannot(Action.Delete, 'Trailer', { companyId });
  }
  ...
  if (userRoles.includes(RoleEnum.REPORT_VIEWER)) {
    builder.can(Action.View_Report, 'Trailer', { companyId });
  }
